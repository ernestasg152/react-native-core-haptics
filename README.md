...
**Can I tune the feel?** Yes — either pick a named preset with `tickStyled('soft' | 'strong')` or pass raw parameters with `tickCustom({ intensity, sharpness })` (iOS full; Android picks the closest preset). The `tick()` hot path keeps the defaults (0.45 / 0.6 — matches `UISelectionFeedbackGenerator`).
